...
const G: Point = Point {
    x: U512([0x343280d6115c1d21, 0x4a03c1d356c21122, 0x6bb4bf7f321390b9, 0xb70e0cbd, 0x0, 0x0, 0x0, 0x0]),
    y: U512([0x44d5819985007e34, 0xcd4375a05a074764, 0xb5f723fb4c22dfe6, 0xbd376388, 0x0, 0x0, 0x0, 0x0]),
};
const P: U512 = U512([0x0000000000000001, 0xffffffff00000000, 0xffffffffffffffff, 0xffffffff, 0x0, 0x0, 0x0, 0x0]);
const A: U512 = U512([0xfffffffffffffffe, 0xfffffffeffffffff, 0xffffffffffffffff, 0xffffffff, 0x0, 0x0, 0x0, 0x0]);
const N: U512 = U512([0x13dd29455c5c2a3d, 0xffff16a2e0b8f03e, 0xffffffffffffffff, 0xffffffff, 0x0, 0x0, 0x0, 0x0]);
...
impl JacobianPoint {
    ...
    fn mul(&self, other: U512) -> JacobianPoint {
        ...
        let mut precomp : [JacobianPoint; (1 << (WINDOW_SIZE-1))] = [JacobianPoint::zero(); (1 << (WINDOW_SIZE-1))];
...
impl Point {
    ...
    fn mul(&self, other: U512) -> Point {
        return self.to_jacobian().mul(other).to_affine();
    }
}
...
impl Key {
    ...
    pub fn get_private_hex(&self) -> String {
        return format!("{:0>64x}", self.private);
    }
    pub fn get_public_hex(&self) -> String {
        return format!("{:0>64x}", (U512::one() << (2*KEY_SIZE+2)) | (self.public.x << KEY_SIZE) | self.public.y);
    }
